...
class GradientBoostForecaster(TreeForecaster):
    def __init__(self, config: Dict[str, Any]):
        super().__init__('gradient_boost', config)
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> 'GradientBoostForecaster':
...
def create_gradient_boost_forecaster(config: Dict[str, Any] = MODEL_CONFIGS['gradient_boost']) -> GradientBoostForecaster:
    """
    Create an instance of GradientBoostForecaster with the specified configuration.
    
    Args:
        config (Dict[str, Any]): Configuration for the GradientBoostForecaster.
    
    Returns:
        GradientBoostForecaster: An instance of the GradientBoostForecaster.
    """
...
if __name__ == "__main__":
    ...
    # Prepare data for GradientBoost
    ...
    gb_forecaster = create_gradient_boost_forecaster()
    gb_forecaster.fit(X_train, y_train)
    
    # Make predictions
    predictions = gb_forecaster.predict(X_test)
    point_predictions, lower_bound, upper_bound = gb_forecaster.predict_interval(X_test)
    
    print(f"GradientBoost predictions shape: {predictions.shape}")
    ...
    cv_scores = gb_forecaster.cross_validate(X_train, y_train)
    print(f"\nCross-validation RMSE scores: {cv_scores}")
    print(f"Mean RMSE: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})")
    
    # Save the model
    gb_forecaster.save()
    print(f"GradientBoost model saved to {SAVE_LOAD_CONFIG['model_save_path']}/gradient_boost_model.joblib")
